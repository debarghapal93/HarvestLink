...
export default function BuyerPane() {
  ...
  // ── Optimized: Memoize Aggregated Stats ──
  const { totalDemand, totalMatched, avgPrice } = useMemo(() => {
    if (!buyerDemand.length) return { totalDemand: 0, totalMatched: 0, avgPrice: '0.0' };
    const tDemand  = buyerDemand.reduce((s, d) => s + (d.requestedQty || 0), 0);
    const tMatched = buyerDemand.reduce((s, d) => s + (d.matchedQty || 0), 0);
    const average  = (buyerDemand.reduce((s, d) => s + (d.targetPrice || 0), 0) / buyerDemand.length).toFixed(1);
    return { totalDemand: tDemand, totalMatched: tMatched, avgPrice: average };
  }, [buyerDemand]);

  // ── Optimized: Memoize Filtered Cards List ──
  ...
  const handleRadiusClick = useCallback(() => {
    setRadiusIdx(prev => {
      const next = (prev + 1) % RADII.length;
      addToast(`🔍 Showing: ${RADII[next]}`, 'info');
      return next;
    });
  }, [addToast]);

  const handleOpenPoolClick = useCallback(() => {
    addToast('🔗 Opening full Aggregation Pool marketplace…', 'info');
  }, [addToast]);
